...
describe('#getPublicPrediction', () => {
  describe('if given goals, status and username', () => {
    test('returns a string with the prediction"', () => {
      expect(getPublicPrediction(0, 0, 'Match Finished', 'Sifan')).toBe(`Sifan's voorspelling: 0 - 0`);
      expect(getPublicPrediction(null, null, 'Match Finished', 'Sifan')).toBe('Geen voorspelling');
      expect(getPublicPrediction(null, null, 'Not Started', 'Sifan')).toBe(`Wedstrijd nog niet gespeeld.`);
      expect(getPublicPrediction(3, 3, 'Not Started', 'Sifan')).toBe(`Wedstrijd nog niet gespeeld.`);
    });
  });
});
...
describe('#getPrediction', () => {
  describe('if given goals, status, timestamp, display and username', () => {
    ...
    // 1910206200 = Sat Jul 13 2030
    test('returns a string with the prediction"', () => {
      expect(getPrediction(0, 0, 'Match Finished', 1610206200, 'private', 'Sifan')).toBe(
        getPrivatePrediction(0, 0, 'Match Finished', 1610206200),
      );
      expect(getPrediction(3, 3, 'Not Started', 1910206200, 'private', 'Sifan')).toBe(
        getPrivatePrediction(3, 3, 'Not Started', 1910206200),
      );
      expect(getPrediction(0, 0, 'Match Finished', 1610206200, 'public', 'Sifan')).toBe(
        getPublicPrediction(0, 0, 'Match Finished', 'Sifan'),
      );
      expect(getPrediction(3, 3, 'Not Started', 1910206200, 'public', 'Sifan')).toBe(
        getPublicPrediction(3, 3, 'Not Started', 'Sifan'),
      );
    });
  });
});
...
describe('#getOutCome', () => {
  describe('if give status, goals and timestamp', () => {
    ...
    test('returns a string with the outcome or time of the match"', () => {
      ...
      expect(getOutCome('Time to be defined', null, null, 1910206200)).toBe('t.b.a.');
      expect(getOutCome('Not Started', null, null, 1910206200)).toBe(getTimeFromTimeStamp(1910206200));
    });
  });
});
